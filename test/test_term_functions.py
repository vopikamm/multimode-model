...
class TestTerms:
    ...
    def test_zonal_pressure_gradient_loopbody(self):
        ...
        assert swe._zonal_pressure_gradient_loop_body(
                eta, g, dx, i, j, ni, nj) == -1
        assert swe._zonal_pressure_gradient_loop_body(
                eta, g, dx, ni-1, j, ni, nj) == 9
    ...
    def test_iteration(self):
        ...
        assert np.all(swe._iterate_over_grid_2D(
    ...
    def test_zonal_pressure_gradient(self):
        ...
        assert np.all(swe.zonal_pressure_gradient(state, grid, params).eta.data == np.zeros(x.shape))
        assert np.all(swe.zonal_pressure_gradient(state, grid, params).v.data == np.zeros(x.shape))
    ...
    def test_meridional_pressure_gradient(self):
        ...
        assert np.all(swe.meridional_pressure_gradient(state, grid, params).eta.data == np.zeros(y.shape))
        assert np.all(swe.meridional_pressure_gradient(state, grid, params).u.data == np.zeros(y.shape))
        assert np.all(swe.meridional_pressure_gradient(state, grid, params).v.data == result)
    ...
    def test_zonal_divergence(self):
        ...
        assert np.all(swe.zonal_divergence(state, grid, params).u.data == np.zeros(x.shape))
        assert np.all(swe.zonal_divergence(state, grid, params).v.data == np.zeros(x.shape))
    ...
    def test_meridional_divergence(self):
        ...
        assert np.all(swe.meridional_divergence(state, grid, params).u.data == np.zeros(y.shape))
        assert np.all(swe.meridional_divergence(state, grid, params).v.data == np.zeros(y.shape))
